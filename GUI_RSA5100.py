"""     AUTHOR: Braam Otto                         """
"""     ORGANISATION: SARAO                        """
"""     WEB: http://www.braamotto.com/             """
"""     CONTACT: braamotto@gmail.com               """

import wx
import wx.adv
import RSA5115B_Control_Functions as RSA_Control
import numpy as np
import threading
...
class RSA_GUI(wx.Frame):
    ...
    def InitUI(self):        
        ...
        lstmi = wx.MenuItem(runMenu, wx.ID_ANY, 'Frequency &List')
        runMenu.Append(lstmi)
        self.Bind(wx.EVT_MENU, self.OnList, lstmi)

        helpMenu = wx.Menu()
        htu = wx.MenuItem(helpMenu, wx.ID_ANY, '&How to Use Tektronix RSA5100 Control')
        helpMenu.Append(htu)
        ...
        self.acq_bw_cb1=wx.ComboBox(panel_top, -1, value=self.acq_bw_options[4], pos=(0.65*W, 0.15*H), size=(75, -1), choices=self.acq_bw_options, style=wx.CB_READONLY)
        ...
        self.message_txt.AppendText('Using the Tektronix RSA5100 Automated Control Software:\n')
    ...
    def FreqSetup(self, event):         
        ...
        if self.testing == True:
            # Database_Path = 'D:\\Linux_HDD\\Work_Related\\eclipse_workspace\\XPS2_Backup\\RSA5106A_Control_13Nov2013\\src\\'
            Database_Path = 'C:\\RFI Archive\\'
        else:            
            # Database_Path = 'D:\\Dropbox\\Dropbox\\eclipse_workspace\\RSA5106A_Control_13Nov2013\\Ver_0_4_1\\'
            Database_Path = 'C:\\RFI Archive\\'
...
#            self.acq_bw = np.float(self.acq_bw_txt.GetValue())*self.acq_bw_unit
            ...
            if self.freq_list_mode:
                self.num_steps = len(self.freq_list)
            else:
...
#            self.message_txt.AppendText('Marker added to centre frequency>>>\n')
            if self.IDN_model == 'RSA5106A':
                ...
                if self.acq_bw_cb1.GetValue()=='30.0':
                    self.samples = self.acq_length/26.7e-9
                if self.acq_bw_cb1.GetValue()=='60.0':
                    self.samples = self.acq_length/13.3e-9
                if self.acq_bw_cb1.GetValue()=='110.0':
                    self.samples = self.acq_length/6.7e-9
            else:
                if self.acq_bw_cb1.GetValue()=='0.019531':
                    self.samples = self.acq_length/41000e-9
                if self.acq_bw_cb1.GetValue()=='0.039062':
                    self.samples = self.acq_length/20500e-9
                if self.acq_bw_cb1.GetValue()=='0.078125':
                    self.samples = self.acq_length/10200e-9
                if self.acq_bw_cb1.GetValue()=='0.15625':            
                    self.samples = self.acq_length/5100e-9
                if self.acq_bw_cb1.GetValue()=='0.3125':
                    self.samples = self.acq_length/2600e-9
                if self.acq_bw_cb1.GetValue()=='0.625':
                    self.samples = self.acq_length/1300e-9
...
#             self.ask = RSA_Control.GPIB_Display(self.freq, self.data)
    ...
    def HowTo(self, e):
        ...
        self.message_txt.AppendText('4) Acquisition Setup\n')
        self.message_txt.AppendText('5) Acquire Data\n')
        self.message_txt.AppendText('6) Post Processing (optional) is automatic when selected in (2) Parameters Setup\n')
        self.message_txt.AppendText('----------------------------------------------------------\n')
        self.message_txt.AppendText('\n')
    

    def OnList(self, e):
        self.freq_list_mode = True
        self.freq_list = []
        file_read=open('freq_list\\freq_list.txt','r')
        try:
            for line in file_read:
                self.freq_list.append(np.float(line.strip()))
        finally:
            file_read.close()

        print "Spot Frequency List..."
        ...
        self.message_txt.AppendText('Complete Acquisition Setup to Apply to Frequency List>>>\n')
        self.message_txt.AppendText('>>>\n')
        self.acq_setup_btn.Enable()
    ...
    def OnAbout(self, e):        
        description = """ This software automates measurements using the Tektronix RSA5100 Real-Time Spectrum Analyser, 
        as well as post processing integration using IDL software developed by Dr R. T. Lord (SKA-SA). """
        licence = """ """        
        ...
        info.AddDeveloper('Braam Otto - Tektronix RSA5100 Control Software')
        info.AddDeveloper('Richard Lord - IDL Scripting')
...
def get_data(self):
	...
	print self.freq_list_mode
	print "<<<>>>"
	if self.freq_list_mode:
            self.centre_freq = self.freq_list[n]*self.acq_bw_unit
	else:
	...
	if self.freq_list_mode:
            print "Current <Start><Centre><Stop> Frequency: <", self.centre_freq/1e6-(self.acq_bw/1e6)/2, self.centre_freq/1e6, self.centre_freq/1e6+(self.acq_bw/1e6)/2, ">"
	else:
...
#         [self.freq, self.data] = RSA_Control.GPIB_Acquire(self.RSA, self.centre_freq, self.span, self.acq_length, np.int(self.start_number)+n, self.acq_bw, self.database_filename)
	if self.freq_list_mode:
            RSA_Control.GPIB_Acquire(self.RSA, self.centre_freq, self.span, self.acq_length, np.int(self.start_number)+n, self.acq_bw, self.database_filename, self.centre_freq/1e6, 'list')
	else:
            RSA_Control.GPIB_Acquire(self.RSA, self.centre_freq, self.span, self.acq_length, np.int(self.start_number)+n, self.acq_bw, self.database_filename, n, 'sweep')
