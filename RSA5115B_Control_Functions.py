...
def GPIB_Init(resource_name):
    ...
    for a in range(0,np.shape(rm.list_resources(resource_name))[0]):
        print 'Searching for Instrument: ', resource_name
        # print 'Found Instrument: ', vs.get_instruments_list(True)[a], '\n'
        print 'Found Instrument: ', rm.list_resources(resource_name)[a], '\n'
        # if vs.get_instruments_list(True)[a] == resource_name:
        if rm.list_resources(resource_name)[a] == resource_name:
            GPIB_Found = True
            # RSA = vs.instrument(resource_name)
            RSA = rm.get_instrument(resource_name)
            del RSA.timeout    
            # RSA_IDN = vs.instrument(resource_name).query('*idn?')
            RSA_IDN = RSA.query('*idn?')
            [IDN_company, IDN_model, IDN_serial, IDN_firmware]=RSA_IDN.split(',')
            # RSA_OPT = vs.instrument(resource_name).query('*opt?')
            RSA_OPT = RSA.query('*opt?')
...
#     busy=RSA.query('*OPC?').encode('ascii','ignore')        
#     while busy.strip() != '1':    
# #     while RSA.query('*OPC?') != '1':
...
#    print RSA.query('SENSE:ANALYSIS:LENGTH:ACTUAL?')
...
def GPIB_Acquire(RSA, centre_freq, span, acq_time, total_count, acq_bw, base_filename, app, mode):
...
#         print 'TRACE DELAY DONE'
    if mode == 'list':    
        RSA.write('MMEMORY:STORE:IQ "Z:\\'+base_filename+np.str(app)+'MHz.tiq"')
    else:
